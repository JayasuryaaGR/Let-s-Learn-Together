...
if (isset($_POST['upload'])) {
	...
	$allowed = array('mp3', 'rar', 'zip', 'bin', 'vcd', 'xml', 'sql', 'mdb', 
		...
		'tmp', 'mp4', 'mpg', 'mpeg', '3gp', '3g2', 'wmv', 'doc', 'docx', 'pdf',
		'rtf', 'tex', 'txt', 'docs', 'xlsx', 'pptx', 'avi');
	...
	if ($size > 0) {
		if (in_array($actual_ext, $allowed)) {
			if ($error === 0) {
				if ($size < 999999999999){
					...
					$query = "INSERT INTO files(user_id,type,content) 
								VALUES('$session_user_id','$name','$trim_text')";
		...
		<script type="text/javascript"> 
			alert("Ooopsss!! Sorry!! Files of this format cannot be uploaded. Upload a file of commonly used formats. Choose atmost and atleast one File."); 
		</script>
		...
		$session_user_id = $_SESSION['user_id'];
		$query = "INSERT INTO files(user_id,content) 
								VALUES('$session_user_id','$trim_text')";
		$result = mysqli_query($con, $query);
		header('location: post.php');
